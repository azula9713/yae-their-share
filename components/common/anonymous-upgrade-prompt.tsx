      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              You currently have <strong>temporary session</strong>. Your data will be lost when you clear your browser data or use a different device.
            </AlertDescription>
          </Alert>
          ...
          <div className="text-xs text-center text-muted-foreground space-y-1">
            <p>
              ✅ Your existing data will be automatically migrated when you sign in
            </p>
            <p>
              💾 Signed-in users get automatic backup and sync across devices
            </p>
          </div>
        </div>
      </DialogContent>
...
// Hook to control the upgrade prompt
export function useAnonymousUpgradePrompt() {
  const [isOpen, setIsOpen] = useState(false);
  const { data: user } = useGetCurrentUser();

  const showPrompt = (_feature?: string) => {
    if (user?.isAnonymous) {
      setIsOpen(true);
    }
  };
